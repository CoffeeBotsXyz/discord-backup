        ...
        try {
            ...
            if (guild.iconURL()) {
                if (options && options.saveImages && options.saveImages === 'base64') {
                    backupData.iconBase64 = (await axios_1.default.get(guild.iconURL()).then((res) => res.data.buffer())).toString('base64');
                }
                backupData.iconURL = guild.iconURL();
            }
            if (guild.splashURL()) {
                if (options && options.saveImages && options.saveImages === 'base64') {
                    backupData.splashBase64 = (await axios_1.default.get(guild.splashURL()).then((res) => res.data.buffer())).toString('base64');
                }
                backupData.splashURL = guild.splashURL();
            }
            if (guild.bannerURL()) {
                if (options && options.saveImages && options.saveImages === 'base64') {
                    backupData.bannerBase64 = (await axios_1.default.get(guild.bannerURL()).then((res) => res.data.buffer())).toString('base64');
                }
